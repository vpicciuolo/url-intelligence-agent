...
function field<T>(value: T, confidence: number, method: string, sources: string[], observationIds?: string[]): EvidenceField<T> { return { value, confidence: Math.max(0, Math.min(1, confidence)), method, sources: unique(sources), ...(observationIds?.length ? { observationIds: unique(observationIds) } : {}) }; }
...
function resolveName(pages: PageSignal[], provenance?: ProvenanceReport): EvidenceField<string> {
  ...
  if (best) {
    ...
    const observationIds = provenance?.observations.filter(obs => obs.predicate === "name" && typeof obs.rawValue === "string" && obs.rawValue.toLowerCase().replace(/[^a-z0-9]+/g, " ").trim() === best.display.toLowerCase().replace(/[^a-z0-9]+/g, " ").trim()).map(obs => obs.id);
    return field(best.display, confidence, `first-party-extraction:${unique(best.methods).join("+")}`, best.sources, observationIds);
  }
...
function findDescription(pages: PageSignal[], provenance?: ProvenanceReport): EvidenceField<string> | undefined {
  const root = pages[0];
  const rootClaim = provenance?.claims.filter(claim => claim.subject === root.url && claim.predicate === "description" && typeof claim.displayValue === "string").sort((a, b) => b.resolution.confidence - a.resolution.confidence)[0];
  if (rootClaim && typeof rootClaim.displayValue === "string") {
    const observations = provenance?.observations.filter(obs => rootClaim.observationIds.includes(obs.id)) || [];
    return field(rootClaim.displayValue, rootClaim.resolution.confidence, `claim-resolution:${rootClaim.resolution.policy}`, unique(observations.map(obs => obs.source.pageUrl)), rootClaim.observationIds);
  }
  const primary = root.description || pages.find(x => x.description)?.description; if (!primary) return undefined;
  const matching = provenance?.observations.filter(obs => obs.predicate === "description" && obs.rawValue === primary) || [];
  return field(primary, root.description === primary ? 0.9 : 0.76, "first-party-extraction:description-fallback", pages.filter(p => p.description === primary).map(p => p.url), matching.map(obs => obs.id));
}

function findContradictions(pages: PageSignal[], name: EvidenceField<string>, provenance?: ProvenanceReport): string[] {
  ...
  if (provenance) out.push(...provenanceContradictions(provenance));
  return unique(out);
}
...
export async function investigate(rawUrl: string, profileOrOptions: string | InvestigateOptions = process.env.URL_AGENT_PROFILE || "full-intelligence"): Promise<IntelligenceResult> {
  ...
  const provenance = buildProvenanceReport(pages);
  const name = resolveName(pages, provenance); const type = resolveType(pages); const description = findDescription(pages, provenance);
  ...
  const confidenceAssessment = {
    ...
    interpretation: "Extraction confidence measures how strongly observable first-party evidence supports the extracted field. Claim-level provenance separately records the representation, layer, locator, normalization, temporal context and conflict/drift state for each observation. External corroboration measures verified evidence outside the target domain. None of these scores is a probability that every claim is true."
  } as const;
  ...
  const provenanceFingerprint = createHash("sha256").update(JSON.stringify(provenance.claims.map(x => ({ predicate: x.predicate, value: x.displayValue, status: x.status })).sort((a, b) => a.predicate.localeCompare(b.predicate)))).digest("hex");
...
export function compareResults(a: IntelligenceResult, b: IntelligenceResult) {
  ...
  const claimsA = new Map(a.provenance.claims.map(x => [x.predicate, x.displayValue]));
  const claimsB = new Map(b.provenance.claims.map(x => [x.predicate, x.displayValue]));
  const sharedPredicates = [...claimsA.keys()].filter(x => claimsB.has(x));
  const differingClaims = sharedPredicates.filter(x => JSON.stringify(claimsA.get(x)) !== JSON.stringify(claimsB.get(x))).map(predicate => ({ predicate, a: claimsA.get(predicate), b: claimsB.get(predicate) })).slice(0, 100);
  return { meta: a.meta, similar: { entityType: a.entity.type.value === b.entity.type.value, sharedSocials: a.socials.filter(x => b.socials.includes(x)), sharedEmails: a.contacts.emails.filter(x => b.contacts.emails.includes(x)), sharedTechnologies: techA.filter(x => techB.includes(x)), sameCanonicalHost: new URL(a.finalUrl).hostname === new URL(b.finalUrl).hostname, sharedClaimPredicates: sharedPredicates.length, differingClaims }, a: { url: a.finalUrl, name: a.entity.name.value, type: a.entity.type.value, seo: a.seo.score, trust: a.trust.score, provenance: a.provenance.summary, technologies: techA }, b: { url: b.finalUrl, name: b.entity.name.value, type: b.entity.type.value, seo: b.seo.score, trust: b.trust.score, provenance: b.provenance.summary, technologies: techB } };
}
...
export async function runAction(name: string, args: Record<string, any>): Promise<unknown> {
  ...
  if (name === "render_page") { const rendered = await renderUrl(args.url, { captureNetwork: args.captureNetwork !== false }); return { meta: projectMeta(), rendered: rendered ? { url: rendered.url, renderer: rendered.renderer, html: args.includeHtml === false ? undefined : rendered.html, screenshotBase64: rendered.screenshotBase64, networkEvidence: rendered.networkEvidence, blockedRequests: rendered.blockedRequests } : null }; }
  ...
  if (name === "inspect_provenance") return { meta: result.meta, provenance: inspectProvenance(result.provenance, args.predicate, Number(args.limit || 100)), ...(String(args.format || "").toLowerCase() === "prov" ? { prov: exportProvJson(result.provenance) } : {}) };
  if (name === "verify_claim") {
    if (!args.predicate) throw new Error("verify_claim requires predicate");
    if (args.value === undefined) throw new Error("verify_claim requires value");
    return { meta: result.meta, verification: verifyClaim(result.provenance, String(args.predicate), args.value as JsonValue) };
  }
  if (name === "map_site" || name === "deep_crawl") return { meta: result.meta, rootUrl: result.finalUrl, importantPages: result.importantPages, sitemapUrls: result.sitemapUrls, pages: result.pages, provenance: result.provenance.summary };
  if (name === "resolve_entity") return { meta: result.meta, entity: result.entity, confidenceAssessment: result.confidenceAssessment, webResearch: result.webResearch, provenance: inspectProvenance(result.provenance, "name", 25), contradictions: result.contradictions, graph: result.graph };
  ...
  if (name === "audit_seo") return { meta: result.meta, seo: result.seo, provenance: result.provenance.claims.filter(x => ["title", "description", "canonical_url"].includes(x.predicate)) };
  ...
  if (name === "structured_data") return { meta: result.meta, structuredData: structuredDataInventory(result.pages), provenance: result.provenance.claims.filter(claim => claim.observationIds.some(id => result.provenance.observations.find(obs => obs.id === id && ["json_ld", "microdata", "rdfa"].includes(obs.source.layer)))) };
  if (name === "api_discovery") return { meta: result.meta, surfaces: discoverApiSurfaces(result.pages), runtimeEvidence: result.provenance.observations.filter(obs => obs.source.layer === "api") };
  ...
  if (name === "commerce_intelligence") return { meta: result.meta, commerce: extractCommerceSignals(result.pages), provenance: result.provenance.claims.filter(x => /price|currency|availability/i.test(x.predicate)) };
  if (name === "content_freshness") return { meta: result.meta, freshness: contentFreshness(result.pages), provenance: result.provenance.claims.filter(x => /date|published|modified|metric:/i.test(x.predicate)) };
  ...
  if (name === "knowledge_export") return { meta: result.meta, knowledge: exportKnowledge(result), provenance: exportProvJson(result.provenance) };
